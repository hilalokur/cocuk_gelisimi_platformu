              ...
              if (hController.text.isNotEmpty && wController.text.isNotEmpty) {
                ...
                try {
                  ...
                  if (childDoc.exists) {
                    ...
                    DateTime birthDate = (childDoc['birthDate'] as Timestamp)
                        .toDate();
                    ...
                    String cinsiyetStr = (childDoc['gender'] ?? '')
                        .toString()
                        .toLowerCase();
                    String dbCinsiyet =
                        (cinsiyetStr == 'kız' ||
                    ...
                    DocumentSnapshot standardDoc = await FirebaseFirestore
                        .instance
                        .collection('growth_standards')
                        .doc(
                          'ay_$kacAylik',
                        ) // İŞTE SİHİR BURADA! 'ay_3' yerine 'ay_14' vs. gelecek
                        .get();

                    if (standardDoc.exists) {
                      // Artık sadece 'kiz' değil, çocuğun gerçek cinsiyetine göre tabloyu alıyoruz
                      Map<String, dynamic> referansVerileri =
                          standardDoc[dbCinsiyet];
                      double altSinir = (referansVerileri['kilo']['alt'] as num)
                          .toDouble();
                      double ustSinir = (referansVerileri['kilo']['ust'] as num)
                          .toDouble();

                      // Z-Skoru Karşılaştırması
                      if (weight < altSinir) {
                        analizMesaji =
                            "Dikkat: $kacAylik aylık $dbCinsiyet bebek için 3. Persentil (Alt Sınır: $altSinir kg) altında.";
                      } else if (weight > ustSinir) {
                        analizMesaji =
                            "Dikkat: $kacAylik aylık $dbCinsiyet bebek için 97. Persentil (Üst Sınır: $ustSinir kg) üstünde.";
                      } else {
                        analizMesaji =
                            "Harika! $kacAylik aylık $dbCinsiyet bebek için ideal aralıkta.";
                      }
                    }
                  }
                } catch (e) {
