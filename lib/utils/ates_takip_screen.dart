...
class _AtesTakipScreenState extends State<AtesTakipScreen> {
  ...
  void _saglikBakanligiSistemSorgusu() {
    ...
    if (atesDegeri == null) {
      setState(() {
        _bakanlikMesaji =
            "Geçersiz format! Lütfen sadece sayı giriniz (Örn: 37.2).";
      });
      return;
    }
    ...
    Future.delayed(const Duration(milliseconds: 700), () {
      ...
      setState(() {
        ...
        if (atesDegeri < normalAltLimit) {
          _bakanlikMesaji =
              "Sistem Durumu: Doğrulandı (Düşük Ateş)\n"
        ...
        } else if (atesDegeri >= normalAltLimit &&
            atesDegeri <= normalUstLimit) {
          _bakanlikMesaji =
              "Sistem Durumu: Doğrulandı (Normal Ateş)\n"
              "Ölçüm Yeri: $_olcumYeri\n"
              "Bakanlık Standart Aralığı: $normalAltLimit - $normalUstLimit °C\n"
              "Not: Ateş normal seyrediyor. Veriler e-Nabız çocuk takip havuzuna işlendi.";
        } else if (atesDegeri > normalUstLimit &&
            atesDegeri <= (normalUstLimit + 1.0)) {
          _bakanlikMesaji =
              "Sistem Durumu: UYARI (Hafif Ateş)\n"
              "Ölçüm Yeri: $_olcumYeri\n"
              "Bakanlık Standart Aralığı: $normalAltLimit - $normalUstLimit °C\n"
              "Not: Sınır değer aşımı. Çocuğun kıyafetlerini gevşetin, bol sıvı takviyesi yapın ve ölçümü tekrarlayın.";
        } else {
          _bakanlikMesaji =
              "Sistem Durumu: ACİL UYARI (Yüksek Ateş)\n"
              "Ölçüm Yeri: $_olcumYeri\n"
              "Bakanlık Standart Aralığı: $normalAltLimit - $normalUstLimit °C\n"
